...
def estimate_word_width(tsc_word: str, default_char_width=20, all_lowercase=False, previous_word=""):
    ...
    # lecters may be transcribed lower case but written upper case if preceded by a full stop
    mult_first = 1
    try:
        if previous_word[-1] == '.':
            mult_first = 1.3
    ...
    char_width = lambda ch_ind: AVG_CHARSIZES[ch_ind[0]][0] * (1 if not ch_ind[1] == 0 else mult_first)
