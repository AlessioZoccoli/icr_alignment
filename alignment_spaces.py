...
def start_alignment():
    ...
    for fnm in fnms:
        ...
        if stop == "y":
            ...
            print(os.path.isdir(images_path), os.path.isdir(transcriptions_path), os.path.isdir(mapping_path))
            ...
            for row_ind, (tsc_line, (img_line, top_y)) in enumerate(transcr2img):
                            ...
                            try:
                                ...
                                for d_lft, d_rt in drawn:
                                    ...
                                    if start_text[0] < d_lft <= start_text[1]:
                                        print("insert space overlapping with start_text")
                    ...
                    for sp in range(1, len(selected_spaces)):
                        ...
                        # whitest column left
                        try:
                            left = np.argmin(np.count_nonzero(img_line[:, space_left[0]:space_left[1]] == 0, axis=0),
                                             axis=0)
                        except ValueError:
                        ...
                        try:
                            ...
                            write_status = cv2.imwrite(os.path.join(images_path, bbx_name + ".png"),
                                                       img_line[:, left: right+1])
                            if not write_status:
                                print("row {}, word {} not inserted".format(row_ind, words[sp-1]))
                            else:
                                with open(os.path.join(transcriptions_path, bbx_name + ".txt"), "w") as out_tr:
                                    out_tr.write(words[sp-1])
            ...
            # to JSON
            output_file = os.path.join(mapping_path, fnm + "_bbxs2transcription.json")
            with open(output_file, "w") as of:
                json.dump(obj=bboxes2transcript, fp=of, indent=4)
